...
import { Event, EventWithId } from '../../../types'
...
export default function EditEventPage() {
    ...
    // Initialize state with empty values or useEffect to load existing event data
    const [calDate, setCalDate] = useState<Nullable<Date>>();
    ...
    const [existingImgUrl, setExistingImgUrl] = useState<string | null>(null);  // New state for the existing image
    ...
    const populateFormFields = (eventData: EventWithId) => {
        setEventTitle(eventData.title)
        setCalDate(eventData.date)
        setEventLocation(eventData.location)
        setEventFee(eventData.fee || null)
        setEventSum(eventData.summary)
        setEventDesc(eventData.description)
        setIsRecurringInput(!!eventData.isRecurring)

        if (eventData.recurrencePattern) {
            setRecurringType(eventData.recurrencePattern.frequency)
            setRecurringInterval(eventData.recurrencePattern.interval)
            setEndDate(eventData.recurrencePattern.endDate ? new Date(eventData.recurrencePattern.endDate) : null)

            if (eventData.recurrencePattern.frequency === RecurrenceFrequency.Weekly) {
                setSelectedDays(eventData.recurrencePattern.daysOfWeek || [])
            } else if (eventData.recurrencePattern.frequency === RecurrenceFrequency.Monthly) {
                setSelectedDayOfMonth(eventData.recurrencePattern.dayOfMonth)
            }
        }
        ...
        setEndTime(new Date(eventDate.getTime() + 2 * 60 * 60 * 1000)) // Assuming 2 hours duration

        setLocationType(eventData.location.includes('http') ? 'virtual' : 'physical')
    }
    ...
    const updateEvent = async () => {
        ...
        if (response.ok) {
            ...
            router.push('/admin'); // Navigate to the admin page after update
        } else {
    ...
    const renderEventDetails = () => (
        <div className="space-y-6">
            <div>
                ...
                <InputText id="title" name="title" value={eventTitle} onChange={(e) => { setEventTitle(e.target.value) }} className="mt-1 block w-full" required />
            </div>
            <div>
